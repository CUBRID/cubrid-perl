...
my $sth=$dbh->table_info('','','aaa%', 'view   aaa')  or die $dbh->errstr ."  :table_info error\n";
my $table_counter=1;
my $true1=0;
my $true2=0;
...
$dbh->do("drop view aaa_view1;") or die "drop error: $dbh->errstr";
$dbh->do("drop view aaa_view2;") or die "drop error: $dbh->errstr";

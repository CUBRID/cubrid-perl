...
my $sth=$dbh->prepare("insert into doc_t values(?,?);") or die "delete error: $dbh->errstr";

my $bdRs1=$sth->bind_param(1,'id1') or die "bind_param error: $dbh->errstr";
my $bdRs2=$sth->bind_param(2,"CHAR_TO_CLOB('This is a Dog')",SQL_CLOB) or die "bind_param error: $dbh->errstr";

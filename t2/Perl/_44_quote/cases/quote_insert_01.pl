...
my $sql1=sprintf"insert into t1 values (%s);",$dbh->quote("single quote(') test");
my $sql2=sprintf"insert into t1 values (%s);",$dbh->quote("new line(\n) test");
my $sql3=sprintf"insert into t1 values (%s);",$dbh->quote("carrage return (\r) test");
my $sql4=sprintf"insert into t1 values (%s);",$dbh->quote("backslash (\\) test");
my $sql5=sprintf"insert into t1 values (%s);",$dbh->quote("backslash quote(\\') test");

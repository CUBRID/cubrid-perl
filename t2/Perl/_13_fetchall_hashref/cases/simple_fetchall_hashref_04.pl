...
$dbh->do("INSERT INTO doc_t values(1,'passsssssaaaa',25),(2,'dass',40),(3,'p',25);") or die "insert: $dbh->errstr";

my $sth=$dbh->prepare("SELECT * FROM doc_t;");
$sth->execute() or die "execute select: $dbh->errstr\n";
my $ref =$sth->fetchall_hashref('age') or die "all_ref error: $dbh->errstr";

my $value=$ref->{50}->{'name'};

plan tests=>1;
ok(!$value,"fetchall_hashref('age') ok");

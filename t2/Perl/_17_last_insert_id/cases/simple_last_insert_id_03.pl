...
$dbh->do("INSERT into ss VALUES(NULL,'DDD'),(NULL,'EEE'),(NULL,'FFF');") or die "insert error: $dbh->errstrt";
my $id3=$dbh->last_insert_id(undef,undef,'ss',id) or die "last_insert_id error: $dbh->errstr";
#print "last_insert_id is $id3\n";

plan tests=>3;
ok($id1 == 1,"last_insert_id ok");
ok($id2 == 2 ,"last_insert_id ok");
ok($id3 == 4, "last_insert_id ok");

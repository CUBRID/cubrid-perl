...
$dbh->do("CREATE TABLE track(album VARCHAR(10),dsk INTEGER,posn INTEGER, song VARCHAR(255),FOREIGN KEY (album) REFERENCES album(id));");
# or die $dbh->errstr . " : create error\n";

my $expected="CUBRID DBMS Error : (-921) The domain of the foreign key member 'album' is different from that of the primary key member 'id'.";
